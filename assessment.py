...
# dict.setdefault(word, 1).__add__(1) ??
...
def count_words(phrase):
    ...
    words_list = phrase.split() # split phrase by whitespace and extend into list
    words_dictionary = {}
    for word in words_list:
        if word not in words_dictionary:
            words_dictionary.setdefault(word, 1)
        else:
...
def word_length_sorted(words):
    ...
    # Creates dictionary
    for word in words:
        dict.setdefault(len(word), []).append(word)

    # Order Sublists
    for key in dict:
        dict[key].sort()

    # Creates tuples from dictionary
    word_length = [] 
    word_length.extend((dict.items()))
